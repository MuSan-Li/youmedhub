...
- **语音转写专用**：`qwen3-asr-flash`（见 `src/api/asr.ts`，OpenAI 兼容接口）——qwen3.x 系列视频输入为抽帧视觉、不含音频轨，台词列依赖 ASR 前置转写注入提示词。实现：浏览器 WebAudio 提取音轨（decodeAudioData + OfflineAudioContext 重采样 16k mono）→ 30s 分块 WAV → 分批并发转写（块偏移即时间戳）
