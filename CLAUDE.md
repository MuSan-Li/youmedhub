...
- **仅使用阿里百炼 qwen 系列模型**
- 默认模型：`qwen3.8-omni-flash`（全模态，**原生理解视频画面与音频**，视频拆解时免 ASR 前置转写）
- 其他：`qwen3.8-flash`（快速）、`qwen3.8-max`（高质量，支持长视频解析），上一代 `qwen3.7-flash` / `qwen3.7-plus`
...
- 模型选择 UI 展示 qwen3.8-omni-flash（默认）、qwen3.8-flash、qwen3.8-max、qwen3.7-flash、qwen3.7-plus
